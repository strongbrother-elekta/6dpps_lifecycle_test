...
class PPSCtrl_Rtc:
    ...
    def _wait_reached_signal_disappear(self, axis: util_pb2.AxisIndex, timeout=5.0):
        timeout_at = time.time() + timeout
        ret = False
        while True:
            serverTxdata = self._recv_data_rep()
            if serverTxdata is None:
                logging.info("serverTxdata is none")
                del serverTxdata
                gc.collect()
                break
            if time.time() > timeout_at:
                logging.info("wait_reached_signal_disappear timeout")
                del serrecv_track_reachedverTxdata
                gc.collect()
                break
            serverTx = tblservicemode_pb2.ServerTx()
            parse_serverTx_len = serverTx.ParseFromString(serverTxdata)
    ...
    def handle_lateral_axis_message(self,default_timeout=8,cur=1):
        x_track_count = len(self.x.track)
        x_cur=cur
        if cur >= x_track_count: # count move to end.
            self.x.cycle = self.x.cycle - 1
            x_timeout = default_timeout
            x_cur = 0

        else: # start to move next count.
            x_cur += 1
            self.x.count = int(self.x.track[x_cur])
            self.x.count_velocity = int(self.x.track_velocity[x_cur])
                    ...
                    if axes.is_target_reached:
                        ...
                        if axes.axis == util_pb2.AxisIndex.LATERAL:
                            ...
                            if self.x_count == x_timeout:
                                ...
                                self.send_single_axis_count_move_req_x()
                                if not self.wait_reached_signal_disappear_x():
                                    logging.info(f"@@@@@@@@@@@@@@@@@@@@@@@@@ didn't wait for the x returncurrent point is: {x_cur}")
                                    self.send_single_axis_count_move_req_x()
                                    assert self.wait_reached_signal_disappear_x()
                        ...
                        if axes.axis == util_pb2.AxisIndex.LONGITUDINAL:
                            ...
                            if self.y_count == y_timeout:
                                ...
                                if not self.wait_reached_signal_disappear_y():
                                    logging.info(f"@@@@@@@@@@@@@@@@@@@@@@@@@ didn't wait for the y returncurrent point is: {y_cur}")
                                    self.send_single_axis_count_move_req_y()
                        ...
                        if axes.axis == util_pb2.AxisIndex.VERTICAL:
                            if self.is_z_p_linkage:
                                if not p_startmove:
                                    ...
                                    if self.z_count == z_timeout:
                                        ...
                                        if not self.wait_reached_signal_disappear_z():
                                            logging.info(f"@@@@@@@@@@@@@@@@@@@@@@@@@ didn't wait for the z1 returncurrent point is: {z_cur}")
                                            self.send_single_axis_count_move_req_z()
                                ...
                                if self.z_count == z_timeout:
                                    ...
                                    self.send_single_axis_count_move_req_z()
                                    if not self.wait_reached_signal_disappear_z():
                                        logging.info(f"@@@@@@@@@@@@@@@@@@@@@@@@@ didn't wait for the z2 returncurrent point is: {z_cur}")
                                        self.send_single_axis_count_move_req_z()
                                        assert self.wait_reached_signal_disappear_z()
                        ...
                        if axes.axis == util_pb2.AxisIndex.ROLL:
                            ...
                            if self.r_count == r_timeout:
                                ...
                                if not self.wait_reached_signal_disappear_r():
                                    logging.info(f"@@@@@@@@@@@@@@@@@@@@@@@@@ didn't wait for the r returncurrent point is: {r_cur}")
                                    self.send_single_axis_count_move_req_r()
                            ...
                            if p_startmove:
                                ...
                                if self.p_count == p_timeout:
                                    ...
                                    if not self.wait_reached_signal_disappear_p():
                                        logging.info(f"@@@@@@@@@@@@@@@@@@@@@@@@@ didn't wait for the p returncurrent point is: {p_cur}")
                                        self.send_single_axis_count_move_req_p()
                        ...
                        if axes.axis == util_pb2.AxisIndex.ISO:
                            ...
                            if self.iso_count == iso_timeout:
                                ...
                                if not self.wait_reached_signal_disappear_iso():
                                    logging.info(f"@@@@@@@@@@@@@@@@@@@@@@@@@ didn't wait for the iso returncurrent point is: {iso_cur}")
                                    self.send_single_axis_count_move_req_iso()
                                    assert self.wait_reached_signal_disappear_iso()

                logging.info(
                    f"@@@@@@@@@ Left Cycles: {self.bigcycle} Current Cycle Stage - {self.stage} @@@@@@@ x  : {self.x.cycle}, y : {self.y.cycle}, z : {self.z.cycle}, r : {self.r.cycle},  p : {self.p.cycle},  iso : {self.iso.cycle}"
                )

                if not self.configfile is None:
                    logging.info(f"#############################config file: {ret}")
    ...
    def _rtc_send_stop_move_req(self, axis: util_pb2.AxisIndex):
        ctlSuite = tblctlsuite_pb2.ServerRx()
        serverRx = tblctl_pb2.ServerRx()
        serverRx.stop_move_req.axes.append(axis)
        ctlSuite.ctl.CopyFrom(serverRx)
        logging.info(f"parse_ctlsuite : {ctlSuite}")
        ctlSuite_protobuf_data = ctlSuite.SerializeToString()
        self._send_data_req(ctlSuite_protobuf_data)
        del ctlSuite_protobuf_data
        del ctlSuite
        gc.collect()
    
    def rtc_send_stop_move_req_x(self):
        return self._rtc_send_stop_move_req(self.x.axis)

    def rtc_send_stop_move_req_y(self):
        return self._rtc_send_stop_move_req(self.y.axis)
    
    def rtc_send_stop_move_req_z(self):
        return self._rtc_send_stop_move_req(self.z.axis)

    def rtc_send_stop_move_req_r(self):
        return self._rtc_send_stop_move_req(self.r.axis)

    def rtc_send_stop_move_req_p(self):
        return self._rtc_send_stop_move_req(self.p.axis)

    def rtc_send_stop_move_req_iso(self):
        return self._rtc_send_stop_move_req(self.iso.axis)

    def _rtc_send_move_to_end_req(self, axisIndex: util_pb2.AxisIndex, dir, speed):
        ctlSuite = tblctlsuite_pb2.ServerRx()
        serverRx = tblctl_pb2.ServerRx()
        axisSpeed = tblctl_pb2.AxisSpeed()
        axisSpeed.axis = axisIndex
        axisSpeed.dir = dir
        axisSpeed.speed = int(speed)
        serverRx.start_move_endpos_req.axes.append(axisSpeed)
        ctlSuite.ctl.CopyFrom(serverRx)
        logging.info(f"parse_ctlsuite : {ctlSuite}")
        ctlSuite_protobuf_data = ctlSuite.SerializeToString()
        self._send_data_req(ctlSuite_protobuf_data)
    ...
    def rtc_send_move_to_end_req_x(self, dir, speed):
        return self._rtc_send_move_to_end_req(self.x.axis, dir, speed * 100)

    def rtc_send_move_to_end_req_y(self, dir, speed):
        return self._rtc_send_move_to_end_req(self.y.axis, dir, speed * 100)

    def rtc_send_move_to_end_req_z(self, dir, speed):
        return self._rtc_send_move_to_end_req(self.z.axis, dir, speed * 100)

    def rtc_send_move_to_end_req_r(self, dir, speed):
        return self._rtc_send_move_to_end_req(self.r.axis, dir, speed * 10)

    def rtc_send_move_to_end_req_p(self, dir, speed):
        return self._rtc_send_move_to_end_req(self.p.axis, dir, speed * 10)

    def rtc_send_move_to_end_req_iso(self, dir, speed):
        return self._rtc_send_move_to_end_req(self.iso.axis, dir, speed * 10)

    def _rtc_send_move_to_point_req(self, axisIndex: util_pb2.AxisIndex, pos, speed):
        ctlSuite = tblctlsuite_pb2.ServerRx()
        serverRx = tblctl_pb2.ServerRx()
        axis = util_pb2.Axis()
        axis.axis = axisIndex
        axis.pos = pos
        axis.speed = int(speed)
        serverRx.start_move_setpoint_req.axes.append(axis)
        ctlSuite.ctl.CopyFrom(serverRx)
        logging.info(f"parse_ctlsuite : {ctlSuite}")
        ctlSuite_protobuf_data = ctlSuite.SerializeToString()
        self._send_data_req(ctlSuite_protobuf_data)
    ...
    def rtc_send_move_to_point_req_x(self, pos, speed):
        return self._rtc_send_move_to_point_req(self.x.axis, pos, speed * 100)

    def rtc_send_move_to_point_req_y(self, pos, speed):
        return self._rtc_send_move_to_point_req(self.y.axis, pos * 100, speed * 100)

    def rtc_send_move_to_point_req_z(self, pos, speed):
        return self._rtc_send_move_to_point_req(self.z.axis, pos, speed * 100)

    def rtc_send_move_to_point_req_r(self, pos, speed):
        return self._rtc_send_move_to_point_req(self.r.axis, pos, speed * 10)

    def rtc_send_move_to_point_req_p(self, pos, speed):
        return self._rtc_send_move_to_point_req(self.p.axis, pos, speed * 10)

    def rtc_send_move_to_point_req_iso(self, pos, speed):
        return self._rtc_send_move_to_point_req(self.iso.axis, pos, speed * 10)

    def _rtc_check_reach_target(self, axis: util_pb2.AxisIndex, pos=0, timeout=16.0, diff = 0.5 * 100):
        timeout_at = time.time() + timeout
        ret = False
        while True:
            ctlSuite = tblctlsuite_pb2.ServerTx()
            parse_ctlSuite_len = ctlSuite.ParseFromString(self._recv_data_rep())
            serverTx = ctlSuite.ctl
            if serverTx is None:
                break
            if time.time() > timeout_at:
                logging.info("wait_reached_signal_appear timeout")
                ...
                gc.collect()
                break
            if serverTx.WhichOneof("Msg") == "start_move_endpos_rep":
                logging.info(f"serverTx.start_move_endpos_rep : {serverTx.start_move_endpos_rep}")
                if serverTx.start_move_endpos_rep.status != util_pb2.ReplyStatus.OK:
                    del serverTx
                    del ctlSuite
                    gc.collect()
                    break
            if serverTx.WhichOneof("Msg") == "table_stat_push":
                for axes in serverTx.table_stat_push.axes:
                    if axes.axis == axis:
                        if abs(axes.primary_position - pos) <= diff:
                            logging.info(f"serverTx.service_status_push.axes : {axes}")
                            del serverTx
                            del ctlSuite
                            gc.collect()
    ...
    def rtc_check_reach_target_x(self, pos, timeout):
        return self._rtc_check_reach_target(self.x.axis, pos * 100, timeout, 0.5 * 100)

    def rtc_check_reach_target_y(self, pos, timeout):
        return self._rtc_check_reach_target(self.y.axis, pos * 100, timeout, 0.5 * 100)

    def rtc_check_reach_target_z(self, pos, timeout):
        return self._rtc_check_reach_target(self.z.axis, pos * 100, timeout, 0.5 * 100)

    def rtc_check_reach_target_r(self, pos, timeout):
        return self._rtc_check_reach_target(self.r.axis, pos * 10, timeout, 0.1 * 10)

    def rtc_check_reach_target_p(self, pos, timeout):
        return self._rtc_check_reach_target(self.p.axis, pos * 10, timeout, 0.1 * 10)

    def rtc_check_reach_target_iso(self, pos, timeout):
        return self._rtc_check_reach_target(self.iso.axis, pos * 10, timeout, 0.5 * 10)

    def rtc_set_integrity_mode(self, mode):
        ctlSuite = tblctlsuite_pb2.ServerRx()
        serverRx = tblctl_pb2.ServerRx()
        serverRx.integrity_mode_ind.mode = mode
        ctlSuite.ctl.CopyFrom(serverRx)
        logging.info(f"parse_ctlsuite : {ctlSuite}")
        ctlSuite_protobuf_data = ctlSuite.SerializeToString()
        self._send_data_req(ctlSuite_protobuf_data)
    ...
    def rtc_check_integrity_mode_reply(self, timeout=16.0):
        timeout_at = time.time() + timeout
        ret = False
        while True:
            ctlSuite = tblctlsuite_pb2.ServerTx()
            ...
            if time.time() > timeout_at:
                ...
                gc.collect()
                break
            if serverTx.WhichOneof("Msg") == "integrity_mode_rep":
                logging.info(f"serverTx.integrity_mode_rep : {serverTx.integrity_mode_rep}")
                if serverTx.integrity_mode_rep.status == util_pb2.ReplyStatus.OK:
                    del serverTx
                    del ctlSuite
                    gc.collect()
                    ret = True
    ...
    def rtc_send_iec_61217_req(self, lateral, longitudinal, vertical, pitch, roll, iso):
        ctlSuite = tblctlsuite_pb2.ServerRx()
        serverRx = tblctl_pb2.ServerRx()
        serverRx.iec61217_asu_req.pos_lateral = lateral
        serverRx.iec61217_asu_req.pos_longitudinal = longitudinal
        serverRx.iec61217_asu_req.pos_vertical = vertical
        serverRx.iec61217_asu_req.pos_pitch = pitch
        serverRx.iec61217_asu_req.pos_roll = roll
        serverRx.iec61217_asu_req.pos_iso = iso
        ctlSuite.ctl.CopyFrom(serverRx)
        logging.info(f"parse_ctlsuite : {ctlSuite}")
        ctlSuite_protobuf_data = ctlSuite.SerializeToString()
        self._send_data_req(ctlSuite_protobuf_data)
    ...
    def rtc_check_asu_61217_reply(self, timeout=16.0):
        timeout_at = time.time() + timeout
        ret = False
        while True:
            ctlSuite = tblctlsuite_pb2.ServerTx()
            ...
            if time.time() > timeout_at:
                ...
                gc.collect()
                break
            if serverTx.WhichOneof("Msg") == "iec61217_asu_rep":
                logging.info(f"serverTx.iec61217_asu_rep : {serverTx.iec61217_asu_rep}")
                if serverTx.iec61217_asu_rep.status == util_pb2.ReplyStatus.OK:
                    del serverTx
                    del ctlSuite
                    gc.collect()
                    ret = True
