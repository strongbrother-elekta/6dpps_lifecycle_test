...
def set_log(filename):
    ...
    time_handler = TimedRotatingFileHandler(filename=filename, when='D', interval=1, backupCount=10)
...
logger=set_log('../logs/cycle_count_move/hyx_test.log')
        ...
        while self.xindex<track_count:
            ...
            if self.next_move_x==True and self.send_stop==False:
                ...
                self.x.count_velocity = int(self.x.track_velocity[self.xindex])
                 #logging.info(f"------axis_move_handler_x-------- {self.xindex}/{track_count}/{self.send_stop}")  
        ...
        while self.yindex<track_count:
            ...
            if self.next_move_y==True and self.send_stop==False:
                ...
                self.y.count_velocity = int(self.y.track_velocity[self.yindex])
                 #logging.info(f"------axis_move_handler_y-------- {self.yindex}/{track_count}/{self.send_stop}")  
...
def axis_move_handler_z(self,threadName):
    ...
    while self.z.cycle>0:
        ...
        while self.zindex<track_count:
            #logging.info(f"------axis_move_handler_z thread have start index:{index} / {track_count}")  
            if self.next_move_z==True and self.axis_z_move_done==False and self.send_stop==False:
                self.z.count = int(self.z.track[self.zindex])
                self.z.count_velocity = int(self.z.track_velocity[self.zindex])
                 #logging.info(f"------axis_move_handler_z-------- {self.zindex}/{track_count}/{self.send_stop}")  
...
def axis_move_handler_p(self,threadName):
    ...
    while self.p.cycle>0:
        ...
        while self.pindex<track_count:
            ...
            if self.next_move_p==True and self.axis_z_move_done==True and self.send_stop==False:
                self.p.count = int(self.p.track[self.pindex])
                self.p.count_velocity = int(self.p.track_velocity[self.pindex])
                 #logging.info(f"------axis_move_handler_p-------- {self.pindex}/{track_count}/{self.send_stop}")  
    ...
    while self.r.cycle>0:
        ...
        while self.rindex<track_count:
            ...
            if self.next_move_r==True and self.send_stop==False:
                self.r.count = int(self.r.track[self.rindex])
                self.r.count_velocity = int(self.r.track_velocity[self.rindex])
                 #logging.info(f"------axis_move_handler_r-------- {self.rindex}/{track_count}/{self.send_stop}")  
...
class PPSCtrl_Count:
    def __init__(self, traceclient: TraceClient):
        ...
        self.x = TableMoveParamCount(util_pb2.AxisIndex.LATERAL)
        self.y = TableMoveParamCount(util_pb2.AxisIndex.LONGITUDINAL)
        self.z = TableMoveParamCount(util_pb2.AxisIndex.VERTICAL)
        self.r = TableMoveParamCount(util_pb2.AxisIndex.ROLL)
        self.p = TableMoveParamCount(util_pb2.AxisIndex.PITCH)
        self.iso = TableMoveParamCount(util_pb2.AxisIndex.ISO)
    ...
    def _send_single_axis_pos_move_req(self, movePara: TableMoveParamCount):
    ...
    def _send_single_axis_count_move_req(self, movePara: TableMoveParamCount):
        serverRx = tblservicemode_pb2.ServerRx()
        ...
        logging.info(f"send count req by axis :axis:{axisReq.axis} reqid:{axisReq.req_id} count: {axisReq.count}")
        serverRx_protobuf_data = serverRx.SerializeToString()
        self._send_data_req(serverRx_protobuf_data)
        del serverRx_protobuf_data
        serverRx_protobuf_data=None
            ...
            if msg is not None:
                ...
                if serverTx.WhichOneof("Msg") == "data":
                    ...
                    if serverTx1.WhichOneof("Msg") == "service_status_push":
                        #logging.info(f"------------------------------------service_status_push with status_id : {serverTx1.service_status_push.status_id}");
                        for axes in serverTx1.service_status_push.axes:
                            if axes.is_target_reached:
                                ...
                                if axes.axis == util_pb2.AxisIndex.LATERAL:
                                    if abs(self.x.count - axes.primary_count) < 300 and self.next_move_x==False:
                                        self.next_move_x=True
                                        logging.info(f"------------------------------------x : {self.next_move_x},sef.x.count:{self.x.count},target:{axes.primary_count} abs:{abs(self.x.count - axes.primary_count)}")
                                if axes.axis == util_pb2.AxisIndex.LONGITUDINAL:
                                    if abs(self.y.count - axes.primary_count) < 300 and self.next_move_y==False:
                                        self.next_move_y=True
                                        logging.info(f"------------------------------------y : {self.next_move_y},sef.y.count:{self.y.count},target:{axes.primary_count} abs:{abs(self.y.count - axes.primary_count)}")
                                if axes.axis == util_pb2.AxisIndex.PITCH:
                                    if abs(self.p.count - axes.primary_count) < 500 and self.next_move_p==False:
                                        self.next_move_p=True
                                        logging.info(f"------------------------------------p : {self.next_move_p},sef.p.count:{self.p.count},target:{axes.primary_count} abs:{abs(self.p.count - axes.primary_count)}")
                                if axes.axis == util_pb2.AxisIndex.VERTICAL:
                                    if abs(self.z.count - axes.primary_count) < 1000 and self.next_move_z==False:
                                        self.next_move_z=True
                                        logging.info(f"------------------------------------z : {self.next_move_z},sef.z.count:{self.z.count},target:{axes.primary_count},{abs(self.z.count - axes.primary_count)}")  
                                if axes.axis == util_pb2.AxisIndex.ROLL:
                                    if abs(self.r.count - axes.primary_count) < 300 and self.next_move_r==False:
                                        self.next_move_r=True
                                        logging.info(f"------------------------------------r : {self.next_move_r},sef.r.count:{self.r.count},target:{axes.primary_count} abs:{abs(self.r.count - axes.primary_count)}")
                                if axes.axis == util_pb2.AxisIndex.ISO:
                                    if abs(self.iso.count - axes.primary_count) < 300 and self.next_move_iso==False:
                                        self.next_move_iso=True
                                        logging.info(f"------------------------------------iso : {self.next_move_iso},sef.iso.count:{self.iso.count},target:{axes.primary_count} abs:{abs(self.iso.count - axes.primary_count)}")                 
                    ...
                    #   logging.info(f"serverTx.fault_report")
